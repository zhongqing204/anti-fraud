  ...
  <div style="background: #f5f7fa; min-height: 100vh; padding-bottom: 50px">
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px 0; margin-bottom: 30px">
      <div style="width: 60%; margin: 0 auto">
        ...
        <div style="color: rgba(255,255,255,0.9); font-size: 16px">查看您点赞过的内容</div>
      </div>
    </div>
    ...
    <div style="width: 60%; margin: 0 auto">
        ...
        <div v-for="item in data.tableData" :key="item.id" style="margin-bottom: 15px; background: white; border-radius: 10px; padding: 25px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); transition: all 0.3s; cursor: pointer" @mouseenter="$event.currentTarget.style.boxShadow='0 4px 16px rgba(0,0,0,0.12)'" @mouseleave="$event.currentTarget.style.boxShadow='0 2px 8px rgba(0,0,0,0.08)'" @click="goToDetail(item)">
          ...
          <div style="margin-top: 15px; display: flex; gap: 40px; color: #999; font-size: 14px">
            <div style="display: flex; align-items: center; gap: 5px">
              ...
              <span style="color: #409EFF">{{ item.likeCount || 0 }}</span>
            </div>
            <div style="display: flex; align-items: center; gap: 5px">
      ...
      <div v-else style="text-align: center; padding: 80px; color: #999; background: white; border-radius: 10px">
        ...
        <div style="font-size: 18px">暂无点赞记录</div>
        <div style="font-size: 14px; margin-top: 10px; color: #ccc">为您喜欢的内容点赞吧</div>
      </div>
    </div>
  </div>
</template>
...
const loadArticleLikes = () => {
  request.get('/likes/selectAll', {
    params: { userId: data.user.id }
  }).then(res => {
    if (res.code === '200') {
      const likes = res.data || []
      data.likeArticleIds = likes.filter(l => l.articleId).map(l => l.articleId)
      
      if (data.likeArticleIds.length === 0) {
        data.tableData = []
        data.total = 0
        return
      }
...
const loadVideoLikes = () => {
  request.get('/likes/selectAll', {
    params: { userId: data.user.id }
  }).then(res => {
    if (res.code === '200') {
      const likes = res.data || []
      data.likeVideoIds = likes.filter(l => l.videoId).map(l => l.videoId)
      
      if (data.likeVideoIds.length === 0) {
        data.tableData = []
        data.total = 0
        return
      }
...
const loadPublicityLikes = () => {
  request.get('/likes/selectAll', {
    params: { userId: data.user.id }
  }).then(res => {
    if (res.code === '200') {
      const likes = res.data || []
      data.likePublicityIds = likes.filter(l => l.publicityId).map(l => l.publicityId)
      
      if (data.likePublicityIds.length === 0) {
        data.tableData = []
        data.total = 0
        return
      }
...
const loadActivityLikes = () => {
  request.get('/likes/selectAll', {
    params: { userId: data.user.id }
  }).then(res => {
    if (res.code === '200') {
      const likes = res.data || []
      data.likeActivityIds = likes.filter(l => l.activityId).map(l => l.activityId)
      
      if (data.likeActivityIds.length === 0) {
        data.tableData = []
        data.total = 0
        return
      }
...
const loadArticlesByPage = () => {
  ...
  const pageArticleIds = data.likeArticleIds.slice(start, end)
  ...
  Promise.all(promises).then(results => {
    ...
    data.total = data.likeArticleIds.length
  })
}
...
const loadVideosByPage = () => {
  const start = (data.pageNum - 1) * data.pageSize
  const end = start + data.pageSize
  const pageVideoIds = data.likeVideoIds.slice(start, end)
  ...
  Promise.all(promises).then(results => {
    ...
    Promise.all(countPromises).then(videosWithCounts => {
      ...
      data.total = data.likeVideoIds.length
    })
  })
}
...
const loadPublicitiesByPage = () => {
  const start = (data.pageNum - 1) * data.pageSize
  const end = start + data.pageSize
  const pagePublicityIds = data.likePublicityIds.slice(start, end)
  ...
  Promise.all(promises).then(results => {
    data.tableData = results.filter(res => res.code === '200' && res.data).map(res => res.data)
    data.total = data.likePublicityIds.length
  })
}
...
const loadActivitiesByPage = () => {
  const start = (data.pageNum - 1) * data.pageSize
  const end = start + data.pageSize
  const pageActivityIds = data.likeActivityIds.slice(start, end)
  ...
  Promise.all(promises).then(results => {
    data.tableData = results.filter(res => res.code === '200' && res.data).map(res => res.data)
    data.total = data.likeActivityIds.length
  })
}
