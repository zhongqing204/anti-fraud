  ...
  <div>
    <div style="background-color: #f8f8f8; padding: 20px">
      <div style="width: 60%; margin: 0 auto">
        <div style="font-size: 20px; font-weight: bold">我的点赞（{{ data.total }}）</div>
      </div>
    </div>
      ...
      <div class="card" style="margin-bottom: 10px; padding: 20px" v-for="item in data.tableData" :key="item.id">
        ...
        <div style="margin-top: 15px; display: flex; gap: 40px; color: #999; font-size: 14px">
          <div style="display: flex; align-items: center; gap: 5px; cursor: pointer" @click.stop="toggleLike(item)">
            <img src="@/assets/images/点赞.png" alt="点赞" style="width: 16px; height: 16px" :style="{ filter: 'none', opacity: 1 }">
...
const load = () => {
  ...
  request.get('/likes/selectAll', {
    ...
    if (res.code === '200') {
      ...
      data.likeArticleIds = likes.map(l => l.articleId)
